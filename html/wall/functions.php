...
class Functions {
            ...
            if (@strpos($link[0], 'vimeo')) {
                ...
                if (isset($id)) {
                    /* $curl = Functions::getCurl();
                      ...
                      $page = curl_exec($curl);
                      echo $link[0]; */
                    $html = '<iframe src="//player.vimeo.com/video/' . $id . '?portrait=0&amp;color=ffffff" width="100%" height="300" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></p>';
    ...
    public static function addSoundcloudWidgets($text) {
        ...
        foreach ($match as $link) {
            ...
            if (@strpos($link[0], 'soundcloud')) {
                $url = 'https://api.soundcloud.com/resolve?url=' . urlencode($link[0]) . '&_status_code_map%5B302%5D=200&_status_format=json&client_id=7987da7711368f3a846afc8f6cee5121&app_version=d50a6b87';
                $curl = Functions::getCurl();
                ...
                if (isset($json->location)) {
                    ...
                    $html = '<iframe width="100%" height="450" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url=' . urlencode($code) . '&auto_play=false&hide_related=true&show_playcount=true&visual=true"></iframe>';
                    $text = str_replace($link[0], $html, $text);
                }
            }
        }
    ...
    public static function addReverbnationWidgets($text) {
        preg_match_all('#\bhttps?://[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/))#', $text, $match);
        foreach ($match as $link) {
        ...
        foreach ($match as $link) {
            ...
            if (@strpos($link[0], 'youtube')) {
                ...
                if (isset($id[1])) {
                    ...
                    $html = '<iframe width="100%" height="300" src="//www.youtube.com/embed/' . $id[1] . '" frameborder="0" allowfullscreen></iframe>';
                    $text = str_replace($link[0], $html, $text);
                }
            }
        }
...
public static function addBandCampWidgets($text) {
        preg_match_all('#\bhttps?://[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/))#', $text, $match);
        ...
        foreach ($match as $link) {
            ...
            if (@strpos($link[0], 'bandcamp')) {
                ...
                $text = '<iframe style="border: 0; width: 100%; height: 470px;" src="' . $url . '" seamless>
<a href="' . $text . '">' . $bandDes . '</a></iframe>';
            }
        }
        return $text;
    }
	
	 public static function addWorldStarHipHopWidgets($text) {
        preg_match_all('#\bhttps?://[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/))#', $text, $match);
        foreach ($match as $link) {
//echo @strpos($link[0]); die();
            if (@strpos($link[0], 'worldstarhiphop')) {
                ...
                if (isset($page[1])) {
                    ...
                    $player = $player[0];
                    // echo $Wurl; die();
                    $text = '<iframe width="100%" height="300" frameborder="0" src="' . $player . '" allowfullscreen></iframe>';
                }
            }
        }
        return $text;
    ...
    public static function addMediaWidgest($text) {
		...
		$text = Functions::addBandCampWidgets($text);
		$text = Functions::addWorldStarHipHopWidgets($text);
        return $text;
    }
    ...
    public static function getUrlData() {
        ...
        foreach ($doc->getElementsByTagName('meta') as $meta) {
            if ($meta->getAttribute('property') == 'og:image') {
                $firstimage = $meta->getAttribute('content');
            }
        ...
        if ($status['http_code'] == 200) {
			...
			if($firstimage !=''){
			 array_unshift($images,$firstimage);
			 }
            $images = array_merge(array_unique($images), array());

            echo json_encode(array('title' => $title, 'description' => $description, 'contenttype' => $type, 'images' => $images));
        }
    }

	 public static function getUrlMetaData() {
     //   echo $_POST['url'];

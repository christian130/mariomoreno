...
$insta_result = $instagram->getUserFeed(); //$instagram->getUserMedia();//getUserFeed();//getUserMedia();
$users_details = $instagram->getUser($twitterid);
?>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <div class="main_content">
        ...
        <div class="hommidIG main" id="Ig_Home_feeds">
            <div class="twt-wall-tw-info-title" id="igtabs" style="margin-bottom:20px;">
                <div class="TwtLogo">
                    <a href="#"><img src="images/instagram1.jpg" width="55px;" height="11px"></a>
            ...
            <input type="hidden" name="ig_menu_active" id="ig_menu_active" value="ig_home_wall" />
            <div id="insta_feeds">
...
<?php } ?>
    ...
    <script>
        var containerIdig = '#postedComments';
        var tabsIdig = '#igtabs';

        $(document).ready(function(){
            // Preload tab on page load
            if($(tabsIdig + ' DIV.current A').length > 0){
                loadTab($(tabsIdig + ' DIV.current A'));
            }

            $(tabsIdig + ' A').click(function(){
                //$("").val();
                if($(this).attr("href") == 'insta_userwall.php'){
                    $("#ig_menu_active").val("ig_user_wall");
                }else if($(this).attr("href") == 'insta_followers.php'){
                    $("#ig_menu_active").val("ig_user_followers");
                }else if($(this).attr("href") == 'insta_following.php'){
                    $("#ig_menu_active").val("ig_user_following");
                }
                if($(this).parent().hasClass('current')){ return false; }
                $(tabsIdig + ' DIV.current').removeClass('current');
        ...
        function loadTabig(tabObj){
//            if(!tabObj || !tabObj.length){ return; }
//            $(containerIdig).addClass('loading');
//            $(containerIdig).fadeOut('fast');

            $(containerIdig).load(tabObj.attr('href'), function(){
//                $(containerIdig).removeClass('loading');
//                $(containerIdig).fadeIn('fast');
            });
        }
        $(document).ready(function() {
            $('.fancybox').fancybox();
            ...
            $(window).data('ajaxready', true).scroll(function(e) {
                ...
                if  ($(window).scrollTop() >= parseInt(distanceTop)){
                    ...
                    var lastid = $(".timelineItem:last").attr("data-id"); //$(".grid:first").attr('data');
                    // alert(lastid);

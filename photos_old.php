...
if ($_POST['func'] == "listPhotos") {
    ...
    foreach ($paths as $path) {
        ...
        $text.='<a class="mid2wall" href="photo_detail.php?id=' . $path['id'] . '" ><img  style="width: '.$tamAtual.'; height: '.$tamAtual.'; border-radius:px; margin: 2px 0 0 2px;"  src="' . $path['path'] . '"/></a>';
        $i++;
...
if ($_POST['func'] == "remove_open") {
    ...
    foreach ($paths as $path) {
        ...
        $text.='<a class="mid2wall" href="photo_detail.php?id=' . $path['id'] . '" ><img  style="width: '.$tamAtual.'; height: '.$tamAtual.'; border-radius:5px; margin: 2px 0 0 2px;"  src="' . $path['path'] . '"/></a>';
        $i++;
        if ($i >= 9 && count($paths) > 9) {
            $text.= ' and ' . (count($paths) - 9).' more photos';
            break;

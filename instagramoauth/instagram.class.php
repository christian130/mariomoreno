...
class Instagram {
    ...
    public function getLoginUrl($scope = array('basic')) {
        if (is_array($scope) && count(array_intersect($scope, $this->_scopes)) === count($scope)) {
            return self::API_OAUTH_URL . '?client_id=' . $this->getApiKey() . '&redirect_uri=' . $this->getApiCallback() . '&scope=basic+likes+relationships+comments&response_type=code';
        } else {
    ...
    public function getUserMoreFeed($limit = 20, $oauth_token_more, $next_id) {
        return $this->_makeCall('users/self/feed?access_token=' . $oauth_token_more . '&count=0&max_id=' . $next_id, true, array('count' => $limit));
    }

    public function getUserMoreFeeds($limit = 20, $oauth_token_more, $next_id) {
        return $this->_makeCall('users/self/feed?access_token=' . $oauth_token_more . '&count=0&min_id=' . $next_id, true, array('count' => $limit));
    }
    ...
    public function getUserMoreMedia($id = 'self', $limit = 20, $oauth_token_more, $next_id) {
        return $this->_makeCall('users/' . $id . '/media/recent?access_token=' . $oauth_token_more . '&count='.$limit.'&max_id=' . $next_id, true, array('count' => $limit));
    }
    ...
    public function getUserFollowing($id, $oauth_token_more, $next_id, $limit = 0) {
        return $this->_makeCall('users/' . $id . '/follows', true, array('access_token' => $oauth_token_more, 'cursor' => $next_id, 'count' => $limit));
    }
    ...
    public function getUserFollowers($id, $oauth_token_more, $next_id, $limit = 0) {
        return $this->_makeCall('users/' . $id . '/followed-by', true, array('access_token' => $oauth_token_more, 'cursor' => $next_id, 'count' => $limit));
    }
     ...
     * @return mixed
     */
    public function getMedia($id,$oauth_token) {
        return $this->_makeCall('media/' . $id,array('access_token'=>$oauth_token));
    }
    ...
    public function likedMedia() {
        return $this->_makeCall('users/self/media/liked?access_token=48164505.5b610f1.e747adf6a50a439c9a89b07128580020');
    }
